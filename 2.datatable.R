...
#dt <- fread("example_g1e.csv", select = c("EXMD_BZ_YYYY", "RN_INDI", "HME_YYYYMM"))
#dt <- fread("example_g1e.csv", select = 1:5)
#dt <- fread("example_g1e.csv", drop = 6:10)
...
# Enhanced dcast
dt.wide3 <- data.table::dcast(dt.long2,... ~ variable, value.var = c("BP", "VA"))
